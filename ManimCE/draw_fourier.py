...
class FourierCirclesScene(ZoomedScene):
    ...
    def get_y_component_wave(
        self, vectors, left_x=1, color=PINK, n_copies=2, right_shift_rate=5
    ):
        path = self.get_vector_sum_path(vectors)
        wave = ParametricFunction(
            lambda t: op.add(right_shift_rate * t * LEFT, path.function(t)[1] * UP),
            t_min=path.t_min,
            t_max=path.t_max,
            color=color,
        )
        ...
        def update_camera(mob, dt):
            ...
            if mob.start_time <= run_time:
                ...
                self.zoomed_camera.cairo_line_width_multiple = interpolate(
                    ctx, self.camera.cairo_line_width_multiple, alpha_func
                )
                mob.move_to(coord)
            return mob
...
class AbstractFourierOfTexSymbol(FourierCirclesScene):
    def __init__(
        ...
        tex_class=Tex,
        tex_config={"fill_opacity": 0, "stroke_width": 1, "stroke_color": WHITE},
        include_zoom_camera=False,
        scale_zoom_camera_to_full_screen=False,
    ...
    def construct(self):
        ...
        for path in name.family_members_with_points():
            for subpath in path.get_subpaths():
                ...
                self.play(
                    Create(drawn_path), rate_func=linear, run_time=self.time_per_symbol
                )
        ...
        self.play(FadeOut(vectors), FadeOut(circles), Restore(frame), run_time=2)
        self.wait(3)
...
class NCyclesVsRunTime(FourierOfTexSymbol):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(n_vectors=n_vectors, n_cycles=n_cycles, tex=tex, **kwargs)
...
class CustomPosition(FourierOfTexSymbol):
    def __init__(
        ...
        circle_config={"stroke_opacity": 0},
        **kwargs,
    ):
        super().__init__(
            n_vectors=n_vectors,
...
class SVGDefault(FourierFromSVG):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            n_vectors=n_vectors, n_cycles=n_cycles, file_name=file_name, **kwargs
        )
...
class CustomAnimationExample(FourierCirclesScene):
    ...
    def construct(self):
        ...
        c_clef_symbol = SVGMobject(
            "./assets/svg_images/c_clef.svg", **self.fourier_symbol_config
        )
